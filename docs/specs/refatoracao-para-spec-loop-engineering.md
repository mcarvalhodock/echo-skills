...
>
> **v4 — 2026-08-07 (Clean Code universal + refactor não-semântico, durante revisão do `executor`).** Durante revisão humana do `executor/SKILL.md` foi apontado que Clean Code deve prevalecer em tudo — incluindo testes escritos pelo Validador. Ajuste aplicado à tese ([v4 do documento base](../../propostas/spec-loop-engineering.md)) e refletido aqui: (a) critério A7 novo — Executor ganha permissão limitada de refactor não-semântico de testes com verificação antes/depois; Executor ganha poder estrutural de retorno análogo ao do Validador; Validador prescreve Clean Code em testes; (b) caso de borda novo — distinção entre refactor não-semântico e alteração semântica; (c) restrição adicional — refactor não-semântico exige suíte antes/depois idêntica em número de testes, assertions e cobertura por tag.
...
- [ ] **A7.** Clean Code aplica-se universalmente às skills — inclusive à escrita de testes pelo Validator (declarado explicitamente no Passo 3 do `validator/SKILL.md`). Executor ganha permissão limitada de refactor não-semântico de testes (DRY, fixture, nomes, mock de interface) com regras: (a) sem alteração de assertion, comportamento verificado ou cobertura por tag; (b) suíte rodada antes e depois com resultados idênticos (número de testes, assertions, cobertura); (c) cada refactor documentado no handoff. Executor ganha também poder estrutural de retorno análogo ao do Validador — devolve suite ao Validador com registro em `.sle/pressao-metodo.md` quando identifica bug semântico em teste ou passo manual.
...
- Refactor não-semântico de teste pelo Executor (v4) pode esticar a fronteira "semântica vs. não-semântica" — Executor apressado pode declarar como "só extração de fixture" algo que altera setup lógico. Mitigações previstas: handoff documenta refactor; Validador re-inspeciona formato/estrutura na Homologar; Camada 3 sinaliza divergência em cobertura. Basta? Cenário conhecido a acompanhar em Fase O.
...
- **Refactor não-semântico de testes pelo Executor (v4)** exige suíte rodada antes e depois com resultados idênticos (número de testes, número de assertions, cobertura por tag). Divergência = alteração semântica não declarada = reverte + retorno ao Validador.
...
**v4 — 2026-08-07 (Clean Code universal + refactor não-semântico, durante revisão do `executor`):** identificação, durante revisão humana do `executor/SKILL.md`, de que Clean Code deve prevalecer em tudo — inclusive nos testes escritos pelo Validador. Skill `executor` ganhou permissão limitada de refactor não-semântico (novo Passo 3.5), poder estrutural de retorno (bloco de regra de ouro operacional), regras de segurança (suíte antes/depois). Skill `validator` teve Passo 3 ampliado com regra transversal de Clean Code em testes. Tese promovida a v4. Critério adicionado: A7. Caso de borda adicionado: fronteira semântica/não-semântica esticada por Executor apressado. Restrição adicional: refactor não-semântico exige suíte antes/depois idêntica.

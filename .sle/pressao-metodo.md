...
| 2026-08-07 | Escolha de adoção pelo autor do método | Preferência explícita por "algo que envolva" (skills como disciplina) sobre "algo que engesse" (hooks/CI bloqueantes) — enforcement mecânico visto como fricção que atrapalha antes de haver maturidade | (1) autor opta por começar sem hooks e sem CI mesmo tendo toda a infraestrutura instalada e testada; (2) preferência declarada por disciplina interna (invocar skill como método pessoal) sobre enforcement externo (hook bloqueando escrita); (3) sinaliza que o método tem valor mesmo em modo "só skills" — enforcement é escada de maturidade, não pré-requisito | (a) tornar modo "só skills" caminho oficial documentado (não degradação implícita); (b) explicitar na tese que os 3 níveis (skills / +CI / +hooks) são escada, não sequência obrigatória; (c) verificar se as skills têm dependências que impediriam uso puro sem hooks/CI |

| 2026-08-16 | Lote real de 10 specs (`expedicao`, projeto `cla_vendetta`) conduzido por `orquestrar` | **O orquestrador degradou para sessão única em silêncio quando o contexto isolado não estava disponível, e 3 specs fecharam com 68 testes verdes autoatestados** — quem detectou foi o dono da demanda, não o método | (1) `orquestrar` não diz o que fazer quando não há como isolar: o texto manda "abrir sessão nova à mão", que é inerte para um agente conduzindo, e o comportamento emergente virou seguir em frente; (2) a skill é silenciosa sobre paralelismo, e o silêncio foi lido como permissão — 3 `verificar` em paralelo contra um banco só produziram vermelho falso que acusou a entrega em vez do despacho; (3) a tabela de insumos de `verificar` não inclui os artefatos que a spec cita como contrato mas não contém, e a falta do protótipo produziu um `não verificável` falso, que pela tabela de roteamento sobe ao humano como defeito de spec; (4) exigência dura do dono da demanda ("visualmente deve ser IGUAL") ficou em contrato técnico e não em critério numerado, então a spec 04 fechou 15/15 com a tela comprovadamente diferente do protótipo | [`propostas/degradacao-silenciosa-do-orquestrador.md`](../propostas/degradacao-silenciosa-do-orquestrador.md) — 4 correções redigidas para `orquestrar` e `especificar`, mais a lacuna que elas **não** fecham. Nenhuma skill foi alterada; aguarda decisão do autor do método |

| 2026-08-16 | Homologação ao vivo do mini-game `expedicao` (projeto `cla_vendetta`), com o dono da demanda validando o portal e pedindo mudanças pequenas | **O método não tem lane para o modo emenda, e o vazio foi preenchido com a pior opção: sete emendas escritas, testadas e declaradas verdes pela mesma sessão que as conduziu** — nenhuma passou por `verificar` | (1) fazer spec para "tira o ×4 do nome no cinto" custa mais que a mudança, então a única alternativa oferecida pelo método é inaplicável na prática, e o resultado é nenhuma disciplina em vez de disciplina proporcional; (2) numa das emendas quem conduzia **afirmou um vazamento de ouro que não existia** (o teste recém-escrito somava a parcela retida duas vezes) — o erro só caiu por inspeção de bytes, e "corrigi-lo" teria quebrado o escrow que a spec 08 sondou com sete provas; (3) uma recalibragem de uma linha foi aplicada sem rodar a suíte e quebrou seis asserções, descobertas horas depois por acaso; (4) **o dano maior é divergência de spec**: três emendas contradizem o C14 da spec 10, então rodar `verificar` nela hoje reprova com razão formal — e o próximo `codificar` desfaz o que o dono pediu. Registro a favor do método: `FidelidadeDaTelaTest`, criado por exigência de uma verificação independente anterior, reprovou uma das emendas na primeira execução | [`propostas/modo-emenda-na-homologacao.md`](../propostas/modo-emenda-na-homologacao.md) — 4 atribuições novas para `homologar` (limite declarado da emenda, registro na hora, uma `verificar` para o lote, reconciliação da divergência), sem passo novo no ciclo. Forma já decidida pelo autor do método: atribuição de `homologar`, não fase própria. Nenhuma skill foi alterada |

| 2026-08-16 | Ciclo `expedicao` completo (13 specs, 292 testes, homologado) no projeto `cla_vendetta` | **Insumo canônico ausente não interrompe nem se declara: a fase prossegue degradada em silêncio, e o artefato sai com aparência de completo** — quatro ocorrências no mesmo ciclo | (1) contexto isolado indisponível → `orquestrar` caiu em sessão única e 3 specs fecharam com 68 verdes autoatestados; (2) **`dominios.md` não é instalado em `~/.claude/skills/`** (existe só em `echo-skills/`), então as 13 specs do ciclo foram escritas **sem o catálogo canônico de domínios** — funcionou por acidente, porque havia specs anteriores de onde copiar o vocabulário; (3) **`.sle/manifesto.md` ausente no alvo**, então `tdd-aplicavel` caiu no default e nenhum padrão de Clean Code foi declarado — e isto **já estava registrado neste log desde 2026-08-08**, o que prova que registrar não impede: falta enforcement; (4) contra-exemplo que fecha a tese: a ausência de ref base do git **foi** declarada nos 13 vereditos — mas só porque o orquestrador escreveu a exigência à mão em cada despacho, 13 vezes. A skill `verificar` lista o ref base como insumo e não obriga a declarar a falta. **Confiabilidade que depende de alguém lembrar não é confiabilidade.** | [`propostas/insumo-canonico-nao-e-opcional.md`](../propostas/insumo-canonico-nao-e-opcional.md) — regra geral (insumo canônico ausente ou para e sobe, ou é declarado **no artefato**, nunca só na resposta) mais o desdobramento por skill, e o item de instalação: enquanto o instalador não levar `dominios.md` e não gerar o manifesto, todo repositório novo começa degradado sem que a degradação apareça. Formulação do autor do método: *"requisitos canônicos PRECISAM ser cumpridos, independente da existência ou não de arquivos."* Nenhuma skill foi alterada |

**Convergência entre entradas 1 e 2 (Tipo G):** ambas apontam para "modo lite" ou "só skills" como caminho legítimo de adoção. Item (c) do primeiro sinal e item (a) do segundo pedem a mesma coisa. Dois sinais com convergência começam a formar padrão — se houver um terceiro pedindo o mesmo, é hora de agir (spec N2 para modo lite + doc explícita).
